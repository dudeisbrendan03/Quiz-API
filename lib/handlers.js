...
/*Quiz functions*/
handlers.quiz  = function (data, callback) {
    if (['post','put','get','delete'].indexOf(data.method) > -1) {
        handlers.quiz[data.method](data, callback);
    } else {
        callback(405, { status: 405, error: "NOT-ALLOWED", desc: "The method is not allowed.", methods: ['post','put','get','delete'] });
    }
};
...
/**
 * Structure of quiz post
 * 
 *  {
 *      "question": "bla bla",
 *      "answers" : ["Answer","The correct answer (1 in the array as below","another answer"],
 *      "answer"  : 1,
 *      "category": "blasksksk"
 *  }
 */

handlers.quiz.post = function (data, callback) {
    try {
        const   question            = typeof data.payload.question === 'string' && data.payload.question.length > 2 ? data.payload.question : false,
                answers             = data.payload.answers.length === 3 && data.payload.answers.constructor === Array ? data.payload.answers : false,
                answer              = typeof data.payload.answer === 'number' && (data.payload.answer >= 1 && data.payload.answer <= 3) ? data.payload.answer : false,
                category            = typeof data.payload.category === 'string' ? data.payload.category : false,
                questionEntryName   = question.toLowerCase().replace(" ","-"),
                categoryEntryName   = category.toLowerCase().replace(" ","-");

        if (question&&answers&&answer&&category){
            try {
                const questionObj =  {//Create user object
                    question,
                    answers,
                    answer,
                    category,
                ...
                if (fs.existsSync(`.tdat\\quiz\\${String(categoryEntryName)}.json`)) {
                    _data.read('quiz', categoryEntryName, function (err, olddata) {
                        if (!err && olddata) {
                            olddata['questions'].push(questionObj)

                            _data.update('quiz', categoryEntryName, olddata, function (err) {
                                if (!err) {
                                    callback(204);
                                } else {
                                    log.error(err);
                    ...
                    //Create and store that category with the question
                    _data.create('quiz', categoryEntryName, {'questions':[questionObj]}, function (err) {
                        if (!err) {
                            callback(204);
                        } else {
                            log.error("Failed to begin process");
                            console.log(e);
                            callback(500, { status: 500, error: "", desc: "An unknown error has occured." });
                        }
                    });
                ...
                console.log(err);
                callback(500, { status: 500, error: "", desc: "An unclassified error prevented the category from being read. This error is vague due to the fact that this feature is in development. Check that the token is correct and that you have the privileges to perform this operation." });
            }
        }
...
handlers.quiz.get = function (data, callback) {
    const category = typeof data.payload.category === 'string' ? data.payload.category : false;

    if (!category) {
        const { readdirSync } = require('fs');

        const fetchDir  = source =>
            fs.readdirSync(source, { withFileTypes: true })
            .filter(dirent => dirent.isDirectory())
            .map(dirent => dirent.name);//oops, we don't need this anymore

        var categories          = [],
            categoriesOriginal  = fs.readdirSync('.\\.tdat\\quiz');
        
        categoriesOriginal.forEach((categoryCurrent)=>{
            categories.push(categoryCurrent.replace('.json',''))
        });
        console.log(categories)
    }
};
/*Note
 for future development: Start using 3 callbacks
